...
abstract class AbstractRuleExecutionResult implements RuleExecutionResult {
     ...
     * @param pParams
     */
    AbstractRuleExecutionResult(Map<String, String> pParams, Context pInputCtx, Map<String, Rule> pOutFldMap)
            throws RuleException {
        params = pParams;
        if (!params.containsKey(RuleConstants.NANO_START_TIME)) {
            throw new RuleException("Did not find the start time for this rule. This is required for performance metrics. "
                    + "Rule information is "
                    + params.entrySet().stream().map(Map.Entry::toString).collect(Collectors.joining(";")));
        }
    ...
    public String toString() {
        ...
        sb.append("\n\t");
        if (null != params) {
            sb.append(params.entrySet().parallelStream().map(Map.Entry::toString)
